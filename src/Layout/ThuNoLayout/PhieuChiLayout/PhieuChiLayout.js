  ...
  const fetchhoadon = async () => {
    try {
      const response = await fetch(
        `${getApiUrl('domain')}/getphieuchi/${khoID}`
      )
      if (response.ok) {
  ...
  const fetchngay = async () => {
    try {
      const response = await fetch(
        `${getApiUrl(
          'domain'
        )}/getphieuchitheodate/${khoID}?startDate=${beginDate}&&endDate=${endDate}`
      )
      if (response.ok) {
        const data = await response.json()
        setdata(data)
