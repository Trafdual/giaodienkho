  ...
  const fetchData = async (page = 1) => {
    try {
      const response = await fetch(
        `${getApiUrl('domain')}/getnhomkhachhangadmin/${
      ...
      if (response.ok) {
        ...
        setsku(data.data) // Dữ liệu nhóm khách hàng
        setCurrentPage(data.currentPage) // Trang hiện tại
        setTotalPages(data.totalPages) // Tổng số trang
        setTotalResults(data.totalItems) // Tổng số nhóm khách hàng
        setLoading(false)
      } else {
        ...
        <div className='divnhanvien'>
          <div className='detailsnhap'>
            <div className='recentOrdersnhap'>
              ...
              >
                <h4>{selectedItems.length} nhóm khách hàng được chọn</h4>
                ...
                <button
                  ...
                  onClick={() =>
                    selectedItems.length > 0
                      ? setIsLock(true)
                      : alert('Chọn nhóm khách hàng để xóa')
                  }
                >
                ...
                <table className='tablenhap'>
                  <thead className='theadnhap'>
                    <tr>
                      ...
                      <td className='tdnhap'>nhóm khách hàng</td>
                    </tr>
                  </thead>
                  <tbody className='tbodynhap'>
                    {sku.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan='9'>Không có nhóm khách hàng nào</td>
                      </tr>
                    )}
                  </tbody>
                </table>
            ...
            <ModalDelete2
              ...
              content={'Bạn có chắc chắn xóa những nhóm khách hàng này'}
...
export default NhomKhachHangLayout
