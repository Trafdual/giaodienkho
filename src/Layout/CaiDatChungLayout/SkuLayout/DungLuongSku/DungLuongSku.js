  ...
  const fetchdata = async () => {
    try {
      const response = await fetch(
        `${getApiUrl('domain')}/getdungluongsku2/${idsku}`
      )
      if (response.ok) {
          ...
          <button
            ...
            onClick={() => {
              if (selectedIds.length === 0) {
                alert('Chọn một dung lượng sku để cập nhật')
              } else if (selectedIds.length > 1) {
                alert('Chỉ được chọn một dung lượng sku để cập nhật')
              } else {
                setIsOpenEdit(true)
              }
            }}
          ...
          <button
            ...
            onClick={() =>
              selectedIds.length > 0
                ...
                : alert('Chọn một dung lượng sku để xóa')
            }
          >
