  ...
  const fetchData = async (page = 1) => {
    try {
      const response = await fetch(
        `${getApiUrl('domain')}/getloaichungtuadmin/${
      ...
      if (response.ok) {
        ...
        setsku(data.data) // Dữ liệu loại chứng từ
        setCurrentPage(data.currentPage) // Trang hiện tại
        setTotalPages(data.totalPages) // Tổng số trang
        setTotalResults(data.totalItems) // Tổng số loại chứng từ
        setLoading(false)
      } else {
        ...
        <div className='divnhanvien'>
          <div className='detailsnhap'>
            <div className='recentOrdersnhap'>
              ...
              >
                <h4>{selectedItems.length} loại chứng từ được chọn</h4>
                ...
                <button
                  ...
                  onClick={() =>
                    selectedItems.length > 0
                      ? setIsLock(true)
                      : alert('Chọn loại chứng từ để xóa')
                  }
                >
                ...
                <table className='tablenhap'>
                  <thead className='theadnhap'>
                    <tr>
                      ...
                      <td className='tdnhap'>Tên loại chứng từ</td>
                      <td className='tdnhap'>Phương thức</td>
                    </tr>
                  </thead>
                  <tbody className='tbodynhap'>
                    {sku.length > 0 ? (
                      ...
                      <tr>
                        <td colSpan='9'>Không có loại chứng từ nào</td>
                      </tr>
                    )}
                  </tbody>
                </table>
...
export default LoaiChungTuLayout
